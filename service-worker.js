 ...
 *  - the root path "./" (resolves to index.html on most hosts)
 *
 * IMPORTANT: When you ship a new index.html, bump CACHE_VERSION below. This
...
async function notifyClientsOfUpdate(url) {
  ...
  // user-facing prompt. Pathname check is lenient (matches index.html or
  // root "/" which resolves to index.html on most hosts).
  const u = new URL(url);
